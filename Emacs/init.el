	  ...
	  (with-selected-frame target-frame
		(when (display-graphic-p)
		  (let* ((ascii-family "Monaspace Neon NF")
			...
			;; Nerd Font アイコン（doom-modeline / nerd-icons が使う PUA 範囲）
			;; BMP Private Use Area: U+E000-U+F8FF
			(set-fontset-font nil '(#xe000 . #xf8ff)   ascii-spec target-frame 'prepend)
			;; Supplementary PUA-A: U+F0000-U+FFFFF (Nerd Fonts v3+)
			(set-fontset-font nil '(#xf0000 . #xfffff)  ascii-spec target-frame 'prepend)
			;; Supplementary PUA-B: U+100000-U+10FFFF
			(set-fontset-font nil '(#x100000 . #x10ffff) ascii-spec target-frame 'prepend))))))
...
(leaf evil-collection
  ...
  ;; C-SPC / C-@ を全ステートで set-mark-command に割り当て
  ;; evil-define-key は遅延評価のため evil-collection に上書きされる場合がある。
  ;; 各ステートマップに直接 define-key することで確実に優先される。
...
;; C-SPC / C-@ を global-map にも登録（evil ロード前・emacs state フォールバック）
(global-set-key (kbd "C-SPC") #'set-mark-command)
(global-set-key (kbd "C-@")   #'set-mark-command)
;; macOS GUI では C-SPC が C-@ として届かない場合がある。
;; key-translation-map で C-@ → C-SPC に正規化してから set-mark-command に流す。
