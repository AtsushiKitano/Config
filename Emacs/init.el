...
;; Python: プロジェクトの .venv を eglot に認識させてインポートエラーを解消する
(defun my/python-venv-root ()
  "バッファのディレクトリから上方向に .venv を探す。なければ nil。
project-current が nil のプロジェクト（git なし・マーカーなし）でも動作する。"
  (let* ((project (project-current))
         (base (or (when project (project-root project))
                   (locate-dominating-file default-directory ".venv")
                   default-directory))
         (venv (expand-file-name ".venv" base)))
    (when (file-directory-p venv) venv)))

(defun my/python-setup-eglot-venv ()
  "eglot が .venv の Python を使うよう workspace configuration を設定する。
eglot-ensure より先に実行されるよう python-mode-hook に -10 の depth で登録する。"
  (when-let* ((venv (my/python-venv-root))
              (python (expand-file-name "bin/python" venv))
              (venv-dir (file-name-directory (directory-file-name venv))))
    (setq-local python-shell-interpreter python)
    ;; pyright は venvPath（.venv の親ディレクトリ）+ venv（ディレクトリ名）で検索する
    (setq-local eglot-workspace-configuration
...
(defun my/python-eglot-refresh-venv ()
  "eglot 接続後に .venv の workspace configuration を再送する。
モードフック実行時に eglot がまだ起動していなかった場合の補完。"
  (when (derived-mode-p 'python-mode 'python-ts-mode)
    (when-let ((server (eglot-current-server)))
      (my/python-setup-eglot-venv)
      (eglot-signal-didChangeConfiguration server))))

(defun my/python-ensure-pyrightconfig ()
  "プロジェクトに .venv があれば pyrightconfig.json を自動生成する（なければ）。
pyright は workspace/configuration より pyrightconfig.json を優先するため、
これが最も確実に .venv を認識させる方法。"
  (when-let* ((venv (my/python-venv-root))
              (project (project-current))
              (root (project-root project))
              (config-file (expand-file-name "pyrightconfig.json" root)))
    (unless (file-exists-p config-file)
      (let* ((python (expand-file-name "bin/python" venv))
             (python-ver
              (string-trim
               (shell-command-to-string
                (format "%s -c 'import sys; print(\"{}.{}\".format(*sys.version_info[:2]))'"
                        (shell-quote-argument python))))))
        (with-temp-file config-file
          (insert "{\n"
                  "  \"venvPath\": \".\",\n"
                  "  \"venv\": \".venv\",\n"
                  (if (string-match-p "^[0-9]\\+\\.[0-9]\\+$" python-ver)
                      (format "  \"pythonVersion\": \"%s\"\n" python-ver)
                    "  \"pythonVersion\": \"3.12\"\n")
                  "}\n"))
        (message "pyright: pyrightconfig.json を生成しました (Python %s)" python-ver)))))

(dolist (hook '(python-mode-hook python-ts-mode-hook))
  (add-hook hook #'my/python-setup-eglot-venv -10)
  (add-hook hook #'my/python-ensure-pyrightconfig -5))

(add-hook 'eglot-managed-mode-hook #'my/python-eglot-refresh-venv)

(with-eval-after-load 'eglot
  ;; .venv 内の LSP サーバを優先し、なければグローバルの pyright-langserver にフォールバック
  (add-to-list 'eglot-server-programs
               `((python-mode python-ts-mode)
                 . ,(lambda (_interactive)
                      (let* ((project (project-current))
                             (root (or (when project (project-root project))
                                       (locate-dominating-file default-directory ".venv")
                                       default-directory))
                             (candidates '(("pylsp" . nil)
                                           ("pyright-langserver" . ("--stdio"))
                                           ("basedpyright-langserver" . ("--stdio")))))
                        ;; 1. .venv/bin/ 内を優先
                        (or (when root
                              (cl-loop for (bin . args) in candidates
                                       for path = (expand-file-name
                                                   (concat ".venv/bin/" bin) root)
                                       when (file-executable-p path)
                                       return (append (list path) args)))
                            ;; 2. exec-path 上のグローバルインストールを探す
                            (cl-loop for (bin . args) in candidates
                                     for path = (executable-find bin)
                                     when path
                                     return (append (list path) args))))))))
...
(defun my/markdown-preprocess-mermaid (content)
  "CONTENT 内の ```mermaid``` ブロックを mmdc でコンパイルして PNG パス参照に置換する。
返り値は (処理済みMarkdown . 生成されたPNGファイルパスのリスト)。
mmdc が見つからない場合や失敗したブロックはそのまま残す。"
      ...
      (with-temp-buffer
        ...
        (while (re-search-forward "^```mermaid\n\\(\\(?:.*\n\\)*?\\)```" nil t)
          (let* ((code (match-string 1))
                 (in-file (make-temp-file "mmd-in-" nil ".mmd"))
                 (out-file (concat (file-name-sans-extension in-file) ".png")))
            (write-region code nil in-file)
            (if (= 0 (call-process "mmdc" nil nil nil
                                   "-i" in-file "-o" out-file
                                   "-b" "white" "-w" "1200"))
                (progn
                  (push out-file png-files)
                  (replace-match (format "![mermaid](%s)\n" out-file) t t))
              (message "mermaid: 1 ブロックのコンパイルに失敗しました"))
            (delete-file in-file)))
        (cons (buffer-string) png-files)))))

(defun my/markdown-preview--render (buf)
  "BUF のマークダウンを HTML に変換し、ファイルパスを返す。
mermaid コードブロックは mmdc で PNG に変換してから pandoc に渡す。"
  (let* ((content (with-current-buffer buf
                    (buffer-substring-no-properties (point-min) (point-max))))
         (mermaid-result (my/markdown-preprocess-mermaid content))
         (processed-md (car mermaid-result))
         (png-files (cdr mermaid-result))
         (md-tmp (make-temp-file "md-src-" nil ".md"))
         (html-file (make-temp-file "md-preview-" nil ".html"))
         (css-file (make-temp-file "md-css-" nil ".css")))
    (write-region processed-md nil md-tmp)
    (write-region my/markdown-preview-css nil css-file)
    (let ((success (= 0 (call-process "pandoc" nil nil nil
                                      "-f" "gfm"
                                      "-t" "html5"
                                      "--standalone"
                                      "--embed-resources"
      ...
      (delete-file md-tmp)
      ;; pandoc が PNG を HTML に data: URI として埋め込んだ後で削除
      (dolist (f png-files) (ignore-errors (delete-file f)))
      (unless success
...
;; Mermaid: mmdc が使う Puppeteer の Chrome を ~/.cache/puppeteer から自動検出して設定する
(let* ((cache (expand-file-name "~/.cache/puppeteer/chrome-headless-shell"))
       (chrome (when (file-directory-p cache)
                 (cl-loop for dir in (directory-files cache t "mac_arm-" t)
                          for bin = (expand-file-name
                                     "chrome-headless-shell-mac-arm64/chrome-headless-shell" dir)
                          when (file-executable-p bin)
                          return bin))))
  (when chrome
    (setenv "PUPPETEER_EXECUTABLE_PATH" chrome)))

;; Mermaid: .mmd ファイルの編集と PNG プレビュー
(defun my/mermaid-compile-and-preview ()
  "Mermaid バッファを mmdc でコンパイルして PNG をバッファ内に表示する."
  (interactive)
  (let* ((in-file (make-temp-file "mermaid-" nil ".mmd"))
         (out-file (concat (file-name-sans-extension in-file) ".png")))
    (write-region (point-min) (point-max) in-file)
    (message "mermaid: コンパイル中...")
    (set-process-sentinel
     (start-process "mmdc" "*mmdc-output*"
                    "mmdc" "-i" in-file "-o" out-file "-b" "white" "-w" "1600")
     (lambda (_proc event)
       (cond
         ...
         (if (file-exists-p out-file)
             (let ((buf (get-buffer-create "*mermaid-preview*")))
               (with-current-buffer buf
                 (let ((inhibit-read-only t))
                   (erase-buffer)
                   (insert-image (create-image out-file nil nil
                                               :scale (if (fboundp 'image-dpi)
                                                          (/ 96.0 (or (car (image-dpi)) 96))
                                                        1.0)))
                   (goto-char (point-min)))
                 (special-mode)
                 (local-set-key (kbd "q") #'quit-window))
               (pop-to-buffer buf)
               (message "mermaid: プレビュー完了"))
           (message "mermaid: 出力ファイルが見つかりません: %s" out-file)))
        ((string-match "exited abnormally" event)
         (ignore-errors (delete-file in-file))
         (message "mermaid: コンパイル失敗 — *mmdc-output* バッファを確認してください")))))))
...
(leaf mermaid-mode
  ...
  :bind
  (:mermaid-mode-map
   ("C-c C-v" . my/mermaid-compile-and-preview)
   ("C-c C-c" . my/mermaid-compile-and-preview)))
...
(leaf ob-mermaid
  ...
  (with-eval-after-load 'org
    (org-babel-do-load-languages
     ...
     (append (bound-and-true-p org-babel-load-languages)
             '((mermaid . t))))))
...
(custom-set-variables
 ...
 '(org-agenda-files
   '("/Users/kitano-atsushi/org/shared/memo.org"
     "/Users/kitano-atsushi/org/work/tasks.org"
     "/Users/kitano-atsushi/org/personal/tasks.org"
     "/Users/kitano-atsushi/org/book/reading.org"
     "/Users/kitano-atsushi/org/research/tasks.org"))
...
(custom-set-faces
 ...
 ;; Your init file should contain only one such instance.
 ;; If there is more than one, they won't work right.
 '(aw-leading-char-face ((t (:height 3.0))) nil "Customized with leaf in `ace-window' block at `/Users/kitano-atsushi/.emacs.d/init.el'"))
