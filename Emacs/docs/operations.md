...
`vterm-mode`, `dired-mode`, `magit-mode`, `imenu-list-major-mode`, `eat-mode`, `agent-shell-mode`, `xwidget-webkit-mode`
...
| `C-c TAB` | `completion-at-point` | 補完を手動起動 (agent-shell-mode 内) |
| `TAB` / `S-TAB` | `agent-shell-next/previous-item` | 次/前の項目へ移動 (agent-shell-mode 内) |

**補完 (`@` / `/`):** `agent-shell-file-completion-enabled` が `t` のとき自動で `agent-shell-completion-mode` が有効になり、`@` でプロジェクトファイル、`/` でエージェントのスラッシュコマンド（ACP から取得できた場合）を補完する。`@` / `/` を行頭か空白の直後に打つと corfu ポップアップが自動表示。途中から呼ぶ・出ないときは `C-c TAB` で手動起動する。
...
**ページ内検索:** `C-s` で検索語を入力 → 一致箇所へスクロール。`C-s` `RET`（同じ語）を繰り返すと次のヒットへ、`C-r` で後方のヒットへ移動する。
