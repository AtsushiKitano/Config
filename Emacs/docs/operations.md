...
| `C-c o d` | `my/org-daily-today` | 今年の daily を開き今日の日付見出しへ移動・日メタデータ枠を用意 |
...
| `C-,` | `my/avy-goto-string` | 文字列を入力してジャンプ（SKK で日本語入力可） |
...
| `:q` | 終了 (変更があれば拒否) |
| `:q!` | 強制終了 (変更を破棄) |
| `:qa` / `:qa!` | 全ウィンドウを終了 / 強制終了 |
| `:wq` / `:x` / `ZZ` | 保存して終了 |
| `:wqa` | 全バッファを保存して終了 |
| `:e <file>` | ファイルを開く |
| `:e!` | ファイルを再読み込み (変更を破棄) |
| `:enew` | 新規空バッファを開く |
| `:r <file>` | カーソル行の下にファイル内容を挿入 |
| `:r !<cmd>` | コマンドの出力をカーソル行の下に挿入 |
| `:f` / `:f <name>` | 現在のファイル名を表示 / 変更 |

#### バッファ・ウィンドウ操作

| コマンド | 説明 |
|----------|------|
| `:ls` / `:buffers` | バッファ一覧を表示 |
| `:b <name/n>` | 名前または番号でバッファを切り替え |
| `:bn` / `:bp` | 次のバッファ / 前のバッファへ |
| `:bd` / `:bd!` | バッファを閉じる / 強制閉じる |
| `:vs <file>` | 垂直分割でファイルを開く |
| `:sp <file>` | 水平分割でファイルを開く |
| `:close` | アクティブウィンドウを閉じる |
| `:only` | 他のウィンドウを全て閉じる |
| `:tabnew <file>` | 新しいタブでファイルを開く |
| `:tabn` / `:tabp` | 次のタブ / 前のタブへ |
...
| `:1,10d` | 1〜10行目を削除 |
| `:m <n>` | 現在行を行 `n` の後へ移動 |
| `:co <n>` / `:t <n>` | 現在行を行 `n` の後へコピー |
| `:d` | 現在行を削除 (レジスタへ) |
| `:y` | 現在行をヤンク (レジスタへ) |
| `:put <reg>` | レジスタ `<reg>` の内容をペースト |
| `:sort` | 行をソート |
| `:sort!` | 行を逆順ソート |
| `:sort u` | 重複を除いてソート |
...
| コマンド | 説明 |
|----------|------|
| `q<letter>` | マクロ `<letter>` の記録を開始 |
| `q` | マクロ記録を停止 |
| `@<letter>` | マクロ `<letter>` を実行 |
| `@@` | 直前のマクロを再実行 |
| `<n>@<letter>` | マクロを `n` 回実行 |
| `:reg <letter>` | マクロ内容を確認 |
...
| コマンド | 説明 |
|----------|------|
| `:reg` / `:registers` | 全レジスタの内容を表示 |
| `"<letter>y` | テキストをレジスタ `<letter>` にヤンク |
| `"<letter>p` | レジスタ `<letter>` からペースト |
| `"+y` / `"+p` | システムクリップボードへヤンク / ペースト |
...
| コマンド | 説明 |
|----------|------|
| `m<letter>` | 現在位置をマーク `<letter>` に設定 |
| `'<letter>` | マーク行の行頭へジャンプ |
| `` `<letter> `` | マークの正確な位置へジャンプ |
| `''` / ` `` ` | 直前ジャンプ位置へ戻る |
| `'<` / `'>` | 前回選択の先頭 / 末尾へジャンプ |
| `:marks` | 全マークを一覧表示 |
| `:delmarks <letter>` | マークを削除 |
...
| `:! <cmd>` | シェルコマンドを実行 |
| `:!!` | 直前のシェルコマンドを再実行 |
| `:%! <cmd>` | バッファ全体をコマンドでフィルタリング |
| `:'<,'>! <cmd>` | 選択範囲をコマンドでフィルタリング |
| `:sh` | シェルを起動 (`:exit` で戻る) |
...
|----------|------|
| `:set <option>` | オプションを設定 |
| `:set <option>?` | オプションの現在値を表示 |
| `:set no<option>` | オプションを無効化 |
| `:set <option>!` | オプションをトグル |
| `:verbose set <opt>?` | オプションの設定元を表示 |
| `:set all` | 全オプションを表示 |
| `:echo <expr>` | Vim 式を評価して表示 |
| `:messages` | メッセージ履歴を表示 |
| `:version` | Vim/Evil バージョン情報を表示 |
| `:help <topic>` | ヘルプを開く |
...
| `:set wrap` / `:set nowrap` | 折り返しの有効 / 無効 |
| `:set hlsearch` / `:set nohlsearch` | 検索ハイライトの有効 / 無効 |
| `:set ignorecase` | 検索で大文字小文字を無視 |
| `:set smartcase` | 小文字のみの検索で大文字小文字を無視 |
| `:set expandtab` | タブをスペースに展開 |
| `:set tabstop=<n>` | タブ幅を設定 |
| `:set list` / `:set nolist` | 非表示文字の可視化 |

### Emacs State (Evil 無効化モード)

以下のモードは Evil を無効化し、Emacs 標準キーバインドで動作する:
`vterm-mode`, `dired-mode`, `magit-mode`, `imenu-list-major-mode`, `ghostel-mode`, `agent-shell-mode`, `xwidget-webkit-mode`
...
| `P` | **Personal Note** | `personal/<選択ファイル>` の Notes |
| `d` | **Daily** | `personal/daily_YYYY.org` (日付ツリー、年ごとにファイル自動作成) |
...
| キー | コマンド | 説明 |
|------|---------|------|
| `T` | `org-set-tags-command` | タグ設定ダイアログ (Evil Normal) |
| `C-c C-q` | `org-set-tags-command` | タグ設定ダイアログ (標準) |

ダイアログに `:tag1:tag2:` 形式で入力する。または見出し行末に直接記述：

```org
* TODO 統計学の勉強 :research:statistics:
* TODO 論文を読む :research:paper:2026:
```

#### タグの補完候補を登録

ファイル先頭に `#+TAGS:` を追加すると補完が効く：

```org
#+TAGS: research paper statistics ml work personal
```
...
| キー | コマンド | 検索対象 |
|------|---------|---------|
| `m` | Tag マッチ | TODO 以外も含む全エントリ |
| `M` | Tag マッチ | TODO エントリのみ |

**検索式の書き方:**

| 式 | 意味 |
|----|------|
| `research` | `:research:` タグがついたエントリ |
| `research+paper` | research かつ paper |
| `research\|paper` | research または paper |
| `research-DONE` | research タグかつ DONE 以外 |
| `research+TODO="TODO"` | research かつ TODO ステート |

#### タグの継承

親見出しのタグは子見出しに自動継承される：

```org
* 機械学習 :ml:
** TODO モデルを実装する      ← :ml: タグを継承
** TODO 論文を読む :paper:    ← :ml: + :paper:
```

#### ファイルタグ (#+filetags)

ファイル全体にタグを付与する。ファイル作成時に自動挿入される：

```org
#+filetags: :research:
```

`~/org/research/` 配下のファイルには `:research:` が自動付与されるため、`C-c a m` → `research` でディレクトリを横断検索できる。
...
| キー | コマンド | 説明 |
|------|---------|------|
| `C-:` | `avy-goto-char-timer` | 文字を入力してジャンプ先を選択（ASCII 向け） |
| `C-,` | `my/avy-goto-string` | 文字列を入力してジャンプ（SKK で日本語入力可） |
| `C-*` | `avy-resume` | 直前の avy 操作を再開 |
| `M-g M-g` | `avy-goto-line` | 行番号指定でジャンプ |
...
|------|---------|------|
| `C-;` | `consult-buffer` | バッファ・ファイル・履歴を横断検索 |
| `C-c g` | `my/consult-ripgrep-current-dir` | 現在ファイルのディレクトリ配下を全文検索（`grep -r` 相当）→ `RET` で一致箇所へジャンプ |
| `C-c G` | `consult-ripgrep` | プロジェクトルートを全文検索（`C-u` でディレクトリ指定） |
...
> **Note:** `C-c g` / `C-c G` は ripgrep (`rg`) で再帰検索する。ライブプレビューで候補を絞り込み、`RET` でファイル・行へジャンプ。検索語の後に `-- <rgフラグ>` を付けると絞り込める（例 `foo -- -tpy` で Python ファイルのみ）。
...
[ghostel](https://github.com/dakra/ghostel) は libghostty-vt（Ghostty ターミナルの VT エンジン）で動くネイティブモジュール式のターミナルエミュレータ。初回起動時にネイティブモジュールを自動ダウンロードするためビルド手順は不要。

### モードの概要

ghostel には5つの入力モードがあり、コピー/ペーストはモードに応じて操作が異なる。起動直後は **semi-char モード**。

| モード | 説明 | 切り替え |
|--------|------|---------|
| **Semi-char モード** | 起動直後のデフォルト。ほとんどのキーはターミナルへ送信。`ghostel-keymap-exceptions`（既定: `C-c` `C-x` `C-u` `C-h` `M-x` `M-:` `C-\`）は Emacs 側で処理。ペーストは `C-y` | `C-c C-j` |
| **Char モード** | 全キーをターミナルへ送信。vi など full-screen アプリ向け | `C-c M-d` |
| **Line モード** | `M-x shell` 相当。行単位でまとめて送信 | `C-c C-l` |
| **Emacs モード** | `buffer-read-only=t` で全 Emacs キーバインドが使える。出力は流れ続ける。テキストのコピーはこのモードで行う | `C-c C-e` |
| **Copy モード** | 出力を凍結してテキスト選択 | `C-c C-t` |
...
1. ターミナルに表示されているテキストをコピーする
   - **方法A (マウス):** マウスドラッグで選択（既定で copy モードに入る） → kill-ring に入る
   - **方法B (キーボード):** `C-c C-t` で copy モードへ → `C-SPC` でマーク → 移動して `M-w` でコピー → `C-c C-j` で semi-char モードへ戻る
   - **方法C:** `C-c M-w` (`ghostel-copy-all`) でバッファ全体をコピー
2. ターミナルにペーストする
   - semi-char モードで `C-y` → `ghostel-yank` でターミナルへ送信
...
| キー | コマンド | 説明 |
|------|---------|------|
| `C-c t` | `ghostel` | ターミナルを起動 (グローバル) |
| `C-c C-j` | `ghostel-semi-char-mode` | Semi-char モードへ切り替え |
| `C-c M-d` | `ghostel-char-mode` | Char モードへ切り替え |
| `C-c C-l` | `ghostel-line-mode` | Line モードへ切り替え |
| `C-c C-e` | `ghostel-emacs-mode` | Emacs モードへ切り替え |
| `C-c C-t` | `ghostel-copy-mode` | Copy モードへ切り替え |
| `M-RET` | `ghostel-semi-char-mode` | Char モード → Semi-char モードへ |

### コピー操作 (Emacs / Copy モード)

read-only なのでキーボードで cut はできないが、コピーは通常通り動く。

| キー | コマンド | 説明 |
|------|---------|------|
| マウスドラッグ | — | 選択範囲を kill-ring に自動コピー (既定で copy モードへ) |
| `C-SPC` | `set-mark-command` | マークをセット (範囲選択の開始) |
| `M-w` / `C-w` | `ghostel-readonly-copy` | 選択範囲をコピー |
| `C-c M-w` | `ghostel-copy-all` | バッファ全体をコピー |
| `q` / `C-g` | — | read-only モードを抜ける (`ghostel-readonly-fast-exit`) |

### ペースト操作 (Semi-char モード)

| キー | コマンド | 説明 |
|------|---------|------|
| `C-y` | `ghostel-yank` | kill-ring の先頭をターミナルへ貼り付け |
| `M-y` | `ghostel-yank-pop` | kill-ring を選んで貼り付け |
| `S-insert` | `ghostel-yank` | `C-y` と同じ |

### その他の操作 (全モード共通 / `C-c` プレフィックス)

| キー | コマンド | 説明 |
|------|---------|------|
| `C-c C-q` | `ghostel-send-next-key` | 次に押したキーをそのままターミナルへ送信 |
| `C-c C-c` | `ghostel-send-C-c` | `C-c` をターミナルへ送信 (SIGINT 相当) |
| `C-c C-z` | `ghostel-send-C-z` | `C-z` をターミナルへ送信 (SIGTSTP 相当) |
| `C-c C-d` | `ghostel-send-C-d` | `C-d` をターミナルへ送信 (EOF) |
| `C-c M-l` | `ghostel-clear-scrollback` | スクロールバックをクリア |

### ナビゲーション (全モード共通 / カスタム)

| キー | コマンド | 説明 |
|------|---------|------|
| `C-c M-p` | `ghostel-previous-prompt` | 前のシェルプロンプトへジャンプ (OSC 133) |
| `C-c M-n` | `ghostel-next-prompt` | 次のシェルプロンプトへジャンプ (OSC 133) |
| `C-c C-p` | `ghostel-previous-hyperlink` | 前のハイパーリンクへジャンプ |
| `C-c C-n` | `ghostel-next-hyperlink` | 次のハイパーリンクへジャンプ |
...
`ghostel-ime-mode`（ghostel パッケージ同梱）が入力メソッドのコミット（バッファ挿入）を捕捉して PTY へ転送する。eat には無い機能で、これにより SKK 等の Emacs Lisp 入力メソッドが端末へ渡る。ghostel-mode-hook で `ghostel-ime-mode` と `skk-mode` を有効化し、`inhibit-read-only` を local に有効にして変換中の挿入を許可している。
...
| キー | コマンド | 説明 |
|------|---------|------|
| `C-c E` | `emoji-search` | 名前で絵文字を検索して挿入 (SKK 有効中でも使用可) |

### SKK ネイティブ変換

`~/.emacs.d/SKK-JISYO.emoji.utf8` を配置すると、SKK のスペース変換で絵文字が候補として出る。
...
Emacs 再起動後、SKK で日本語や英語の絵文字名を変換するとで絵文字が候補に現れる。
...
[agent-shell](https://github.com/xenodium/agent-shell) は Claude Code を Emacs バッファ内から対話的に呼び出すパッケージ。ACP (Agent Client Protocol) 経由で `claude-agent-acp` プロセスと通信し、コード編集・ファイル操作・許可プロンプトを Emacs UI で扱える。

#### 起動と認証

1. `C-c A` (`agent-shell`) で agent-shell バッファを起動する
2. 認証は `:login t` 設定によりシステムのキーチェーン（CLI の `claude` ログインと共通）を使う。初回のみブラウザ認証が走る
3. バッファは Evil を無効化した **emacs-state** で動作する（`agent-shell-mode` は Evil 無効化モード）。SKK も自動オフ・行番号も非表示になる

> **Note:** カレントバッファ／`default-directory` のあるプロジェクトルートがエージェントの作業ディレクトリになる。特定リポジトリで動かしたいときは、そのリポジトリのファイルを開いた状態で `C-c A` する。

#### 基本操作

- プロンプト行に指示を入力して `RET` で送信する（複数行入力は継続行を入れてから送信）
- エージェントの応答・ツール実行・diff がバッファに順次表示される
- `TAB` / `S-TAB` で応答ブロックや項目を前後に移動できる（`TAB` は補完ではなく項目移動に割当）

| キー | コマンド | 説明 |
|------|---------|------|
| `C-c A` | `agent-shell` | agent-shell バッファを起動 (グローバル) |
| `RET` | — | プロンプトを送信 |
| `TAB` / `S-TAB` | `agent-shell-next/previous-item` | 次 / 前の項目へ移動 |
| `C-c s` | `agent-shell-send-screenshot` | スクリーンショットを撮って送信 |
| `C-c i` | `agent-shell-send-clipboard-image` | クリップボードの画像を送信 |
| `C-c TAB` | `completion-at-point` | 補完を手動起動 |

#### 画像を送る

- `C-c s` — スクリーンショットを撮影してそのままエージェントへ添付（UI のスクショを見せて質問する用途）
- `C-c i` — クリップボードにコピー済みの画像を添付（`Cmd+Shift+4` でクリップボードへコピー → `C-c i`）

#### 補完 (`@` ファイル / `/` スラッシュコマンド)

`agent-shell-file-completion-enabled` が `t` のとき `agent-shell-completion-mode` が自動で有効になる。

- `@` — プロジェクトファイルを補完（`@path/to/file` でファイルをコンテキストに添付）
- `/` — エージェントのスラッシュコマンドを補完（ACP から取得できた場合）
- `@` / `/` を **行頭か空白の直後**に打つと corfu ポップアップが自動表示される
- 途中から呼びたいとき・ポップアップが出ないときは `C-c TAB` で手動起動する

#### 許可プロンプト (Evil キーバインド)

エージェントがファイル編集やコマンド実行の許可を求めたとき、`evil-collection` 由来の 1 キーで応答する（`agent-shell-mode-map` に再定義済み）。

| キー | 動作 |
|------|------|
| `v` | 変更内容の diff を表示 (`permission-view-diff`) |
| `y` | 今回だけ許可 (`permission-allow-once`) |
| `!` | 常に許可 (`permission-allow-always`) |

典型フローは「`v` で diff を確認 → 問題なければ `y` で許可、繰り返すなら `!`」。
...
| キー | コマンド | 説明 |
|------|---------|------|
| `C-c C-v` | `my/markdown-preview` | pandoc + xwidget-webkit (WebKit) でブラウザ風プレビュー表示 |
| `C-c C-x v` | `my/markdown-auto-preview-mode` | 保存時自動プレビュー更新のトグル |
| `C-c C-g` | `browse-url-of-file` | 外部ブラウザでファイルを開く |

> **Note:** プレビューは pandoc で GitHub 風 HTML に変換し、WebKit で描画する（xwidget 対応の GUI Emacs が必要）。
> ` ```mermaid ` コードブロックは Mermaid.js (CDN) で図としてレンダリングされる（インターネット接続が必要）。
...
| キー | コマンド | 説明 |
|------|---------|------|
| `C-c w` | `my/web-search` | Google 検索し、結果を xwidget-webkit (WebKit) で Emacs 内に表示 |
| `o` | `my/xwidget-webkit-hint` | (WebKit バッファ内) Vimium 風リンクヒント。リンクを completing-read で選んで開く |
| `C-s` | `my/xwidget-webkit-search` | (WebKit バッファ内) ページ内テキスト検索 (前方)。同じ語で `RET` を繰り返すと次のヒットへ |
| `C-r` | `my/xwidget-webkit-search-previous` | (WebKit バッファ内) 直近の検索語で後方のヒットへ移動 |

**使い方:**

1. `C-c w` を押す
2. ミニバッファに検索語を入力して `RET`
   - **リージョン選択中**ならその文字列が既定の検索語になる
   - 選択がなければ **カーソル位置のシンボル**が既定値になる（不要なら消して入力）
3. Google の検索結果が WebKit バッファ（ブラウザ同様のリッチ表示）で開く
4. 結果内のリンクを開く（次のいずれか）
   - **マウスクリック**でそのまま遷移
   - **`o`（Vimium 風リンクヒント）**: ページ内リンクを抽出し completing-read で表示 → テキストや URL の一部を入力して絞り込み → `RET` で遷移
   - **キーボード（標準）**: `e` で edit-mode に入り `Tab`/`S-Tab` でフォーカス移動、`RET` で遷移

**ページ内検索:** `C-s` で検索語を入力 → `RET` で一致箇所へスクロール。検索後は `C-s` で次のヒット・`C-r` で後方のヒットへ移動できる（ミニバッファを経由せず連続移動でき、他のキーを押すと検索状態を抜ける）。

> **Note:** 表示には xwidget 対応の GUI Emacs が必要。Markdown プレビューと同じ WebKit セッションを共有するため、両者は同一バッファで切り替わる。
> リンクヒント `o` は現在表示中ページの `<a href>` を JavaScript で収集する（動的に生成されるリンクも取得可）。
> ページ内検索 `C-s` はブラウザ標準の `window.find()` を使う（macOS の xwidget ビルドでは組込みの `xwidget-webkit-isearch-mode` が "Widget has no ongoing search operation" で失敗するための差し替え）。
